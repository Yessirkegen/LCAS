...
### LCAS Warning — замыкание на землю, полноэкранный оверлей
![LCAS Warning](lcas-warning.png)
...
## Вдохновлено авиацией — EICAS / ECAM

Система оповещений LCAS спроектирована по принципам авиационных систем **EICAS** (Engine Indicating and Crew Alerting System, Boeing) и **ECAM** (Electronic Centralised Aircraft Monitor, Airbus), адаптированных для железнодорожного транспорта:

| Авиация (EICAS/ECAM) | LCAS (локомотив) |
|----------------------|------------------|
| Два уровня: WARNING + CAUTION | Три уровня: WARNING + CAUTION + CRITICAL overlay |
| Master Warning/Caution кнопки на панели | Master Warning/Caution панель в UI |
| Голосовые сообщения в кабине | Web Speech API — TTS на русском языке |
| Приоритизация сообщений (верхний = важнейший) | Сортировка алертов по severity |
| Подтверждение экипажем (acknowledge) | Подтверждение пробелом / кнопкой |
| Эскалация если не подтверждено | Эскалация диспетчеру через 30 сек |
| Чеклист рекомендованных действий | Рекомендация в overlay: «НАГРУЗКА СНЯТА АВТОМАТИЧЕСКИ (ДГУ)» |
| Synoptic pages (схемы систем) | Панели температур, давлений, электрики, 3D-модель |

**Почему это важно**: машинист не должен интерпретировать сырые числа. Он должен **слышать** проблему, **видеть** приоритет и **подтвердить** действие — как пилот в кабине Boeing 737.

## Ключевые функции

### Кабина машиниста
- **Health Index** (0–100) — взвешенная оценка по всем параметрам
- **LCAS-оповещения** — CAUTION / WARNING / голосовое TTS / полноэкранный оверлей (авиационный подход)
